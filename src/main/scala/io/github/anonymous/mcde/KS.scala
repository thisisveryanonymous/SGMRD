 ...
 */
package io.github.anonymous.mcde

import io.github.anonymous.index.dimension.D_Rank
import io.github.anonymous.index.{I_Rank, I_Rank_Stream}
import io.github.anonymous.preprocess.DataSet

import scala.annotation.tailrec
import scala.math.{E, pow, sqrt}

// This is basically the HiCS / GMD measure
/**
  ...
  */
case class KS(M: Int = 50, alpha: Double = 0.5, beta: Double = 0.5, var parallelize: Int = 0) extends McdeStats {
  type I = I_Rank
  type D = D_Rank
  val id = "KS"
  ...
  def twoSample(ref: D, indexSelection: Array[Boolean]): Double = {
    ...
    @tailrec def cumulative(n: Int, acc1: Double, acc2: Double, currentMax: Double): Double = {
      if (n == theref.length) currentMax
        ...
        if (theref(n))
          cumulative(n + 1, acc1 + selectIncrement, acc2 + refIncrement, currentMax max math.abs((acc2 + refIncrement) - (acc1 + selectIncrement)))
        else
