...
case class SelectWorst(nbsub: Int) extends Selector {
  require(nbsub >= 1)
  val id = s"Worst-$nbsub"

  def select(subspaces: Array[(Int, (Set[Int], Double))]): Array[Int] = subspaces.sortBy(_._2._2).
    take(nbsub.min(subspaces.length)).map(_._1)

  def observe(searched: Array[Int], current: Array[(Int, (Set[Int], Double))], previous: Array[(Int, (Set[Int], Double))]) = {
    //println(s"Selected worst ${searched mkString ", "} )")
