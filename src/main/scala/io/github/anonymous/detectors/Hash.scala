 ...
 */
package io.github.anonymous.detectors
...
/**
  ...
  *
  * A Scala implementation of RS-Hash
  *
  */
case class Hash() extends FullSpaceOutlierDetector {
  val id = "Hash"
  ...
  def computeScores(data: Array[Array[Double]]): Array[(Int, Double)] = {
    ...
    // Here, take all the dimensions (we assume we are work on the full space, or a subspace of interest already)
    val r = d
    val v = (0 until d).toArray
