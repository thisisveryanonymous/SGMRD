 ...
 */
package io.github.anonymous.monitoring.bandits.external
...
/**
  ...
  */
case class MPTS(scalingstrategy: ScalingStrategy, val narms: Int, var k: Int, threshold: Double) {
  val name = "MP-TS"
  /**
  ...
  def observe(searched: Array[Int], result: Array[(Int, (Set[Int], Double))], current: Array[(Int, (Set[Int], Double))]): Unit = {
    ...
    println(s"STS: avgreward: ${rewards.map(_._2).sum / rewards.length}, searched: ${searched mkString ","}, rewards: ${rewards mkString ","}")
    println(s"STS:results: ${result.map(x => (x._1, x._2._1 mkString ";", "%.2f".format(x._2._2))) mkString ","}")
    println(s"STS:current: ${searched.map(x => (current(x)._1, current(x)._2._1 mkString ";", "%.2f".format(current(x)._2._2))) mkString ","}")
    println(s"STS: estimates: ${beta_params.zipWithIndex.map(x => s"${x._2}:${"%.2f".format(x._1._1 / (x._1._1 + x._1._2))} ") mkString "; "}")
    ...
    println(s"STS: sums: ${sums.sum}, counts: ${counts.sum}")
