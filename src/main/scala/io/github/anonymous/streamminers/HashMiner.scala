 ...
 */
package io.github.anonymous.streamminers

import breeze.stats.mean
import io.github.anonymous.detectors.Hash
import io.github.anonymous.preprocess.DataSet

case class HashMiner(windowsize: Int, stepsize: Int, removeduplicates: Boolean) extends StreamMiner {
  val id = s"Hash"
  ...
  def predict(data: DataSet, subspaces: SearchResult): Array[(Int, Double)] = {
    ...
    val allscores: Array[Array[Double]] = subs.map(x =>
      Hash().computeScores(x.toArray.map(coldata(_)).transpose).map(_._2))
