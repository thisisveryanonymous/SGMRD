 ...
 */
package io.github.anonymous.streamsimulator
...
case class CachedStreamSimulator(dataset: DataRef) extends Simulator {
  ...
  lazy val cache: Array[Array[Double]] = data.columns.transpose
  val data: DataSet = dataset.open() // in CachedStreamSimulator, the dataset is the cache
  val id = dataset.id
  val nbatches: Int = data.nrows
  val npairs: Int = data.ncols
